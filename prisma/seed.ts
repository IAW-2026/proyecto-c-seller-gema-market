...
import { PrismaClient, ProductStatus, ProductCondition } from '../lib/generated/prisma/client';
...
async function main() {
  ...
  console.log(
    `Seed completo: ${1 + extraSellers.length} sellers (1 suspendido), ${categoryNames.length} categorías, ${productSeeds.length + extraProductCount} productos (1 oculto por admin).`,
  );
}
